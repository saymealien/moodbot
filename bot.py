...
async def rating_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "Cancel":
        await update.message.reply_text("❌ Rating cancelled.", reply_markup=MAIN_MENU)
        return ConversationHandler.END
...
async def parameter_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "Cancel":
        await update.message.reply_text("❌ Cancelled.", reply_markup=SETTINGS_MENU)
        return ConversationHandler.END
...
async def delete_parameter_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "Cancel":
        await update.message.reply_text("❌ Delete cancelled.", reply_markup=SETTINGS_MENU)
        return ConversationHandler.END
...
async def timezone_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_input == "Cancel":
        await update.message.reply_text("❌ Timezone setup cancelled.", reply_markup=SETTINGS_MENU)
        return ConversationHandler.END
...
async def set_reminders_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("⏰ Enter reminder time in HH:MM format (e.g., 21:30):")
...
async def donate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "❤️ Love this bot? Support it!\n\n"
        "☕ Buy me a coffee: https://buymeacoffee.com/yourname\n"
        "💳 PayPal: https://paypal.me/yourname\n"
        "🪙 Crypto: [Your crypto address]\n\n"
...
if __name__ == "__main__":
    ...
    # IMPORTANT: Order matters! Export BEFORE Estimate to prevent conflicts
